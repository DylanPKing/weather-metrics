...
class TestGetSensorData:
    ...
    def test_get_missing_sensor_id_returns_422(self, client):
        """Test that missing sensor_id returns 422."""
        response = client.get("/v1", params={"metric": "temperature"})
        assert response.status_code == 422

    def test_get_missing_metric_returns_400(self, client, test_sensor):
        """Test that missing metric returns 422."""
        response = client.get("/v1", params={"sensor_id": test_sensor.id})
        assert response.status_code == 422

    def test_get_invalid_metric_value_returns_422(self, client, test_sensor):
    ...
    def test_get_invalid_statistic_value_returns_422(self, client, test_sensor):
